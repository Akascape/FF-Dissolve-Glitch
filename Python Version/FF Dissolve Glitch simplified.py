...
def Convert(outfile):
    ...
    mc=(mcbox.get()).lower()
    subprocess.call(f'ffmpeg -i "{file}" -filter:v setpts="{sp}"*PTS,minterpolate="fps="{fp}":mb_size=16:search_param=400:vsbmc=0:scd=none:mc_mode="{mc}":me_mode="{mod}":me="{m}"" "{outfile}"', shell=True)
    Log.place_forget()
    root.update_idletasks()
...
Label(root, text="FF DISSOLVE GLITCH", font=("",13),bd=1, fg="#f86604", bg="#FFFFFF").grid()
flabel=Label(root, text="Choose Video", font=("",9), fg="#f86604", bg="#FFFFFF")
flabel.grid()
Vdo=Button(root, width=40,bg="#82CC6C",fg="white",highlightthickness=1,borderwidth=0.2,text="OPEN",relief="groove", command=openfile)
Vdo.grid()
Label(root, text="Choose Mode", font=("",10), bg="#FFFFFF").place(x=42,y=75)
Label(root, text="Choose Type", font=("",10), bg="#FFFFFF").place(x=165,y=75)
Label(root, text="Slow Speed", font=("",10), bg="#FFFFFF").place(x=277,y=120)
Label(root, text="Choose FPS", font=("",10), bg="#FFFFFF").place(x=45,y=120)
Label(root, text="Export Format", font=("",10), bg="#FFFFFF").place(x=160,y=120)
Label(root, text="MC Mode", font=("",10), bg="#FFFFFF").place(x=283,y=75)
modes=["BILAT","BIDIR"]
modebox=ttk.Combobox(root,values=modes, width=10, state="readonly")
modebox.current(0)
modebox.place(x=40,y=95)
me=["TSS","ESA","TDLS","NTSS","FSS","DS","HEXBS","EPZS","UMH"]
mebox=ttk.Combobox(root,values=sorted(me), width=10, state="readonly")
mebox.current(7)
mebox.place(x=160,y=95)
speed=["0.5","1","2","10","20","50"]
spedbox=ttk.Combobox(root,values=speed, width=10)
spedbox.current(1)
spedbox.place(x=270,y=140)
fps=["25", "30", "60"]
fpsbox=ttk.Combobox(root,values=fps, width=10)
fpsbox.current(0)
fpsbox.place(x=40,y=140)
export=["mp4","avi","mov","mkv","wmv"]
exportbox=ttk.Combobox(root,values=export, width=10, state="readonly")
exportbox.current(0)
exportbox.place(x=160,y=140)
mc=["AOBMC","OBMC"]
mcbox=ttk.Combobox(root,values=mc, width=10, state="readonly")
mcbox.current(0)
...
Log=Label(root,text="", font=("",16), bg="#FFFFFF")
